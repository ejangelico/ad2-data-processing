...
def reduce_and_process(dataset, topdir, nevents, nevents_per_file, rogowski_voltage):
	...
	max_evts = max(len(separated_file_lists["pmt"]), len(separated_file_lists["anode"]))
	...
	for i in range(max_evts):
		...
		pmtfile = None
		anodefile = None
		if(i < len(separated_file_lists["pmt"])):
			pmtfile = topdir+dataset+separated_file_lists["pmt"][i]
